...
- **within-burst dynamics** can be modeled in long format
- **between-burst dynamics of process features** can be modeled in wide format
- Mplus does not waste computation time interpreting large gaps between bursts.
...
Dynamic models in Mplus require a **time-interval variable (`TINTERVAL`)** that represents elapsed time between observations.

Instead of counting measurement occasions, this variable encodes **continuous time since the first observation for each participant**.

For example:

- first observation → `0`
- next observation 38 minutes later → `38`

This allows Mplus to correctly interpret temporal precedence.
...
For example, a 30-day gap corresponds to **43,200 minutes**.
...
### Step 1 — Create continuous time variable
